...
public class server implements Runnable {
    ...
    public static String DB_URL = "jdbc:mysql://localhost:3306/Chat";
    ...
    public void run() {
        try {
            ...
            this.conn = DriverManager.getConnection(DB_URL, USER_NAME,
                    PASSWORD);
                ...
                try {
                    ...
                    if (mes.startsWith("Login:")) {
                        ...
                        ResultSet select = stmt.executeQuery(
                                "SELECT ID , Name, Email FROM chat.user WHERE Email='" + Email + "' AND Pass = '"
                                        + Pass + "';");
                        ...
                        ResultSet select = stmt
                                .executeQuery("Select COUNT(Email) From Chat.User WHERE Email = '" + email + "';");
                        this.existmail = 0;
                        while (select.next()) {
                            this.existmail = Integer.parseInt(select.getString(1));
                        }
                        ResultSet select1 = stmt
                                .executeQuery("Select COUNT(Name) From Chat.User WHERE Name = '" + usname + "';");
                        this.existun = 0;
                        while (select1.next()) {
                        ...
                        if (this.existmail > 0) {
                            ...
                            if (this.existun > 0) {
                                ...
                                stmt.executeUpdate(
                                        "INSERT INTO Chat.user(Name, Email , Pass) VALUES ('" + usname + "','"
                                                + email + "','"
                                                + pass + "');");
                                ResultSet select_id = stmt.executeQuery("SELECT ID FROM chat.user WHERE Name = '"
                        ...
                        ResultSet chat = stmt.executeQuery(
                                "SELECT ID, Name_chat FROM Chat_box WHERE ID_User_in_chat = " + this.ID + ";");
                        ...
                        ResultSet his_chat = stmt.executeQuery(
                                "SELECT user_name , message, TIME_send FROM Chat_His WHERE chat_id = "
                                        + submes + ";");
                        ...
                        ResultSet on = stmt.executeQuery(
                                "SELECT User_name FROM Chat_box WHERE ID = "
                                        + submes + ";");
                        ...
                        stmt.executeUpdate("INSERT INTO Chat_His (chat_id,user_id,user_name,message,TIME_send) VALUES ("
                                + this.in_chat + ", " + this.ID + ",'" + this.NAME + "','" + submes + "','"
                        ...
                        ResultSet chat = stmt.executeQuery(
                                "select ID, Name_chat From Chat_box where ID Like '%" + submes
                        ...
                        while (check_ex != false) {
                            ...
                            ResultSet get_ex = stmt
                                    .executeQuery("SELECT count(ID) FROM Chat.Chat_box WHERE ID = '" + n + "'; ");
                            while (get_ex.next()) {
                            ...
                            if (Integer.parseInt(getRe) == 0) {
                                check_ex = false;
                                stmt.executeUpdate(
                                        "INSERT INTO Chat.Chat_box(ID,Name_chat, ID_User_in_chat,User_name,Permission) VALUES ('"
                        ...
                        String get = null;
                        ResultSet get_ex = stmt
                                .executeQuery("SELECT count(ID) FROM Chat.Chat_box WHERE ID = '" + submes + "'; ");
                        while (get_ex.next()) {
                        ...
                        if (Integer.parseInt(get) > 0) {
                            ...
                            if (Integer.parseInt(count) > 0) {
                                ...
                                ResultSet get_namechat = stmt
                                        .executeQuery(
                                                "SELECT Name_chat FROM Chat.Chat_box Where ID = '" + submes + "'");
                        ...
                        if (this.PASS.equalsIgnoreCase(pass)) {
                            if (this.NAME.equals(usname)) {
                                ...
                                stmt.executeUpdate(
                                        "UPDATE chat.user SET Name = '" + usname + "' WHERE ID = '" + this.ID + "'");
                                this.NAME = usname;
                                DO = new DataOutputStream(this.c.getOutputStream());
                        ...
                        if (this.PASS.equalsIgnoreCase(pass)) {
                            if (this.PASS.equalsIgnoreCase(newpass)) {
                                ...
                                this.PASS = newpass;
                                stmt.executeUpdate(
                                        "UPDATE chat.user SET Name = '" + newpass + "' WHERE ID = '" + this.ID + "'");
                                DO = new DataOutputStream(this.c.getOutputStream());
